"""jsonconvert.py builds a JSON file from the data found in the results directory. """
    ...
    if extension_dir != "results.json":
        ...
        for domain_dir_name in domain_name_dir_list:
            ...
            if len(error_dir) > 0:
                ...
                error_latest_file_time = datetime.datetime(
                    int(lt[0]), int(lt[1]), int(lt[2]), int(lt[3]), int(lt[4]), int(lt[5])
                )
                for file in error_dir:
                    ft = file[5:-4].split("_")
                    file_time = datetime.datetime(
                        int(ft[0]), int(ft[1]), int(ft[2]), int(ft[3]), int(ft[4]), int(ft[5])
                    )
            ...
            if len(free_dir) > 0:
                ...
                free_latest_file_time = datetime.datetime(
                    int(lt[0]), int(lt[1]), int(lt[2]), int(lt[3]), int(lt[4]), int(lt[5])
                )
            ...
            if len(taken_dir) > 0:
                ...
                taken_latest_file_time = datetime.datetime(
                    int(lt[0]), int(lt[1]), int(lt[2]), int(lt[3]), int(lt[4]), int(lt[5])
                )
            ...
            data_list = [
                (error_latest_file_time, error_latest_file, "error"),
                (free_latest_file_time, free_latest_file, "free"),
                (taken_latest_file_time, taken_latest_file, "taken"),
            ]
